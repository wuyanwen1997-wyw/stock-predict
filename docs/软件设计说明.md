...
> 专项文档：[algo 纯算法层](./algo/README.md) · [ashare 行情客户端](./ashare/README.md) · [智能选股设计说明](./智能选股设计说明.md) · [数据存储设计说明](./数据存储设计说明.md) · [K 线形态图](./kline/形态图设计说明.md) · [K 线买卖点设计说明](./K线买卖点设计说明.md)
...
flowchart TB
    ...
    subgraph 纯算法层["纯算法层 algo"]
        ...
        BTA["backtest 命中口径"]
        STAT["stats 波动率"]
        BSM["bs_markers MACD B/S"]
    end

    subgraph 基础设施层["基础设施层 Infrastructure"]
        ASH["ashare 行情客户端"]
        MOD["models DTO"]
        RES["本地资源<br/>stocks.json / 权重 JSON / user_data.sqlite / 磁盘缓存"]
    ...
    FAC & TECH & FUSE & SENT & CAPA & BTA & STAT & BSM --> MOD
```
...
| 层级 | 代码位置 | 职责 |
|------|----------|------|
| 表现层 | `src/pages/`、`src/components/` | 首页选股、智能选股、策略配置、概率/信号明细、K 线（含 MACD B/S）/分时、场景路径、回测结果；底栏/侧栏导航 |
| 应用状态层 | `src/stores/stockStore.ts` | 全局选股与分析状态（含 `bsMarkers`）；智能选股条件/进度/结果；策略组合按股票持久化；触发 `runPrediction` / `runSmartScreen` |
| 接口适配层 | `src/services/api.ts` | 将前端类型映射为 Tauri `invoke`；统一错误与参数命名（camelCase） |
| IPC 边界 | `src-tauri/src/commands.rs` | 唯一对外命令面；组装「行情 + 预测 + 回测 + K 线 B/S」与「智能选股」 |
| 领域编排层 | `predictor` / `strategy` / `backtest` / `screener` / `cninfo` / `capital_flow` | 拉数、用例编排、信号源目录与默认组合、进度上报；**不含**核心打分公式 |
| 纯算法层 | `src-tauri/src/algo/` | 因子/技术信号/融合门控/情绪/资金评分/回测口径/MACD B/S；无 HTTP、无磁盘、无 Tauri |
...
| **models** | `src-tauri/src/models.rs` | 前后端共享 DTO：`Stock`、`DailyBar`、`KlinePeriod`、`PredictionResult`、`BsMarker`、`ScreenResult` 等 | serde |
...
| **algo** | `src-tauri/src/algo/` | 纯计算：factor / tech / fuse / sentiment / capital / backtest / stats / **bs_markers** | models |
...
| `analyze_stock` | 行情 + K 线 + 组合预测 + 回测 + MACD B/S 标记（主入口） |
...
flowchart TD
    ...
    D --> BS[algo::bs_markers compute_macd_bs<br/>全量K → 按chart窗口过滤]
    K & L & BS --> M[AnalysisResult]
    M --> N[ProbabilityPanel / SignalBreakdown / KlineChart+B/S / BacktestPanel]
```
...
- chart 窗口：末尾 `max(90, lookback)` 根日 K；**MACD 在全量日 K 上计算**后再按 chart 日期过滤（避免 EMA 截断失真）。
- `bs_markers` 与融合预测 / 回测方向**无关**，仅供主图叠加。
...
| K 线 B/S | `DailyBar[]` → `algo::bs_markers` → `BsMarker[]`（`buy`/`sell`） |
| 智能选股 | `ScreenRequest` → `ScreenProgressEvent` → `ScreenResult` / `ScreenHit` |
| 一体化 | `AnalysisResult { prediction, klines, backtest, bs_markers }` |

前端 `src/types/index.ts` 与 Rust `models.rs` 字段对齐，经 serde JSON 跨 IPC。

#### 3.6 本地持久化

用户态（自选、策略、偏好、盯盘等）落在 `app_data_dir` 下 SQLite，升级不覆盖；localStorage 仅作一次性迁移源；设置页提供导出/导入。

专项见 **[数据存储设计说明.md](./数据存储设计说明.md)**。盯盘规则落库说明亦见 [盯盘助手设计说明.md](./盯盘助手设计说明.md)。
...
| **KlineChart / ScenarioChart** | Canvas 多周期 K 线（MA/量/MACD/RSI，可缩放）+ 日 K MACD B/S；高开/低开演示路径。专项见 [形态图设计说明](./kline/形态图设计说明.md) |
...
flowchart LR
    ...
    JOIN --> BS[algo::bs_markers → chart窗口过滤]
```

`analyze_stock` 是系统主用例：一次请求完成报价刷新、足够长度的日 K、组合预测、滚动回测与 MACD B/S 标记，返回 `AnalysisResult`。
...
flowchart LR
    ...
    BARS[DailyBar] --> FAC & TECH & STAT & BSM
    TITLES[标题+画像] --> SENT
    ARCH[CapitalFlowArchive] --> CAP
    SIGS[多源贡献] --> FUSE
    PREDvsACT[预测 vs 实际] --> BTA
    BSM --> MARK[BsMarker buy/sell]
```
...
| `bs_markers` | K 线 → MACD 金叉/死叉 `BsMarker[]`（**不进 fuse**）；设计见 [K线买卖点设计说明](./K线买卖点设计说明.md) |
...
#### 2.11 K 线主图 B/S（algo::bs_markers）

预测页日 K 叠加 MACD(12,26,9) 金叉/死叉 **B / S**，与融合预测语义解耦；全量日 K 计算后按 chart 窗口过滤。

完整选型、计算口径、编排与前端约定见 **[K 线买卖点设计说明](./K线买卖点设计说明.md)**；符号表见 [algo/api.md §8](./algo/api.md)。
...
| `bs_markers::MIN_BARS` | 35 | MACD B/S 最少日 K 根数 | `algo::bs_markers` |
| MACD 周期 | 12 / 26 / 9 | 快线 / 慢线 / 信号线 | `algo::bs_markers` |
...
| 1.2 | 2026-07-23 | 增补预测页日 K MACD 金叉/死叉 B/S：`algo::bs_markers`、`AnalysisResult.bs_markers`、KlineChart 主图标注；见 §2.11 |
| 1.3 | 2026-07-23 | 将 §2.11 K 线买卖点方案抽离为 [K线买卖点设计说明](./K线买卖点设计说明.md)；本节改为摘要引用 |
| 1.4 | 2026-07-23 | §3.6：用户态迁至 `app_data_dir` SQLite；竞品对照 B+C；localStorage 一次性迁移；导出/导入安全网 |
| 1.5 | 2026-07-23 | 将 §3.6 存储方案抽离为 [数据存储设计说明](./数据存储设计说明.md)；本节改为摘要链接 |
