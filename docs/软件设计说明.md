...
| **KlineChart / ScenarioChart** | 历史日 K + MACD 金叉/死叉 B/S 主图标注；高开/低开演示路径 |
...
| `bs_markers` | K 线 → MACD 金叉/死叉 `BsMarker[]`（**不进 fuse**） |
...
对齐通达信/同花顺默认 **MACD(12,26,9)** 金叉/死叉，仅在预测页日 K 主图叠加 **B / S**，与融合「预测涨跌」语义解耦。
...
4. `bars.len() < 35`（`bs_markers::MIN_BARS`）→ 空列表；同日至多一个点。
...
- DTO：`BsMarker { date, kind: buy|sell }`；`AnalysisResult.bs_markers` 默认空数组兼容旧客户端。
- UI：hover 可显示「买入 B（MACD 金叉）」/「卖出 S（MACD 死叉）」；副标题有标记时附「MACD 金叉/死叉 B/S」。
- **非目标**：自选迷你图、周/分钟 K、均线交叉切换、并入 fuse 权重。

专项 API 表见 [algo/api.md §8](./algo/api.md)。
