...
flowchart TB
    ...
    subgraph IPC边界["IPC 边界 Tauri Commands"]
        CMD["commands.rs<br/>analyze_stock / run_smart_screen / load_stocks / …"]
    end
...
> 智能选股专项设计见 [智能选股设计说明.md](./智能选股设计说明.md)。

#### 1.2 各层职责

| 层级 | 代码位置 | 职责 |
|------|----------|------|
| 表现层 | `src/pages/`、`src/components/` | 首页选股、智能选股、策略配置、概率/信号明细、K 线、场景路径、回测结果展示；底栏导航 |
...
| 领域层 | `predictor` / `strategy` / `backtest` / `screener` / `factor_model` / `message_sentiment` / `capital_flow` | 信号评估、加权融合、方向判定、walk-forward 回测、截面选股 |
...
| 模块 | 路径 | 核心职责 | 主要依赖 |
|------|------|----------|----------|
| **commands** | `src-tauri/src/commands.rs` | Tauri 命令入口；选股加载、一体化分析、智能选股、回测、策略目录、Token 配置 | market, predictor, backtest, screener, strategy |
| **models** | `src-tauri/src/models.rs` | 前后端共享 DTO：`Stock`、`DailyBar`、`PredictionResult`、`ScreenResult` 等 | serde |
...
| **screener** | `src-tauri/src/screener.rs` | 两阶段智能选股：Universe → 硬过滤 → 批量技术打分 / TopN | market, strategy, factor_model |
