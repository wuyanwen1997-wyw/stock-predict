...
| 文档版本 | 1.1 |
| 编制日期 | 2026-07-23 |
| 关联文档 | [软件设计说明.md](./软件设计说明.md) · [盯盘助手设计说明.md](./盯盘助手设计说明.md) · [产品架构升级方案.md](./产品架构升级方案.md) |
| 非目标 | 账号云同步、跨设备实时同步 |
...
| 层级 | 介质 | 内容 | 升级策略 |
|------|------|------|----------|
| **用户态（不可丢）** | `{app_data_dir}/user_data.sqlite` | 自选/股票池、策略组合、持仓、复盘笔记、预测/选股偏好、回看、盯盘规则/开关/预警、`schema_version` | 永不随安装包删除；仅正向迁移 |
...
| **会话态** | 内存 | 当前选中股、分析结果、选股进度、**现价浮盈** | 重启可丢 |
...
### 4.1 schema_version = 1（基线）

| 表 | 说明 |
|----|------|
| `meta(key, value)` | `schema_version`、`localstorage_migrated` |
| `watchlist(...)` | 自选：`code` PK、name/market/sector、`sort_order`、`extra_json`（v2 后只读兼容） |
...
### 4.2 schema_version = 2（股票池）

| 表 | 说明 |
|----|------|
| `pool_groups(id, name, sort_order, kind)` | 分组；默认：关注 / 待买 / 观察 / 已剔除；`kind=mirror` 为持仓镜像 |
| `pool_items(code, group_id, name, market, sector, sort_order, extra_json)` | 池内标的；PK `(code, group_id)` |

**迁移 v1→v2**：种默认分组；将 `watchlist` 行一次性写入「关注」(`g_watch`)；meta `pool_migrated_from_watchlist=1`；此后以 pool 为准，`watchlist` 表保留只读兼容。`save_watchlist` IPC 会写入 `g_watch`。

### 4.3 schema_version = 3（持仓 / 复盘）

| 表 | 说明 |
|----|------|
| `holdings(code PK, name, market, sector, cost, qty, buy_date, note)` | 本地研究仓；现价/浮盈不入用户库 |
| `journal_entries(id, date, code NULL, body, extra_json)` | 复盘笔记 |

**导出**：`ExportBundle.format_version` 与 `CURRENT_SCHEMA_VERSION` 对齐（现为 **3**）；导入时缺省的 pool/holdings/journal 字段按空处理。
...
- 覆盖安装新 APK/桌面包后，自选/股票池、策略组合、持仓、复盘笔记、盯盘规则、回看/预测偏好、Token **仍在**。
- 从仅有 localStorage 的旧版本升级，首次启动自动导入且不丢条目；再经 schema v2/v3 迁移进池/仓表。
- `schema_version` 异常或迁移失败时，不出现「空库当新用户」的静默清空。
- 导出 JSON（format_version=3）后清应用数据再导入，用户态（含 pool/holdings/journal）可恢复。
- 资金流缓存丢失不影响用户态。
...
| 1.1 | 2026-07-23 | 落地产品架构升级：补充 schema v2 股票池、v3 持仓/复盘与导出 format_version=3 |
