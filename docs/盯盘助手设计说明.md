...
| Key | 内容 |
|-----|------|
| `monitor_rules_v1` | 预警规则 |
| `monitor_alerts_v1` | 最近预警（最多 50） |
| `monitor_enabled_v1` | 用户是否希望开启（用于冷启动恢复） |

Rust 侧 `MonitorShared`（Tauri managed state）保存运行中的自选、规则与冷却状态；前端开启前调用 `monitor_sync_config`。
